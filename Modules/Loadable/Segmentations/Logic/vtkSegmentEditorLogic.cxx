  ...
  , RotateWarningInNodeSelectorLayout(true)
{
  ...
  this->EffectNameOrder = std::vector<std::string>{
    "Threshold",           //
    "Paint",               //
    "Draw",                //
    "Erase",               //
...
vtkSegmentEditorLogic::~vtkSegmentEditorLogic()
{
  ...
  for (auto effect : this->RegisteredEffects)
  {
    effect->cleanup();
    effect->Delete();
  }
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorLogic::notifyEffectsOfReferenceGeometryChange(const std::string& geometry)
{
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorLogic::notifyEffectsOfSourceVolumeNodeChange()
{
  for (auto effect : this->RegisteredEffects)
  {
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorLogic::notifyEffectsOfLayoutChange()
{
  for (auto effect : this->RegisteredEffects)
  {
...
bool vtkSegmentEditorLogic::setSourceRepresentationToBinaryLabelmap()
{
  ...
  {
    vtkWarningMacro(" failed: segmentation node is invalid.");
    return false;
  }
  ...
  std::string message = tr("Editing requires binary labelmap source representation, but currently the source representation is %1. "
  ...
  if (answer != QMessageBox::Yes)
  {
    ...
    vtkWarningMacro(" failed: user rejected changing of source representation.");
    return false;
  }
...
void vtkSegmentEditorLogic::setMRMLScene(vtkMRMLScene* newScene)
{
  ...
  this->qvtkReconnect(InteractionNode, interactionNode, vtkCommand::ModifiedEvent, this, onInteractionNodeModified());
...
void vtkSegmentEditorLogic::setInteractionNode(vtkMRMLInteractionNode* interactionNode)
{
  ...
  this->qvtkReconnect(InteractionNode, interactionNode, vtkCommand::ModifiedEvent, this, SLOT(onInteractionNodeModified()));
  InteractionNode = interactionNode;
}

//------------------------------------------------------------------------------
void vtkSegmentEditorLogic::initializeParameterSetNode()
{
  if (!ParameterSetNode)
  {
    return;
  ...
  MRMLNodeModifyBlocker blocker(ParameterSetNode);

  // Set parameter set node to all effects
  for (auto effect : RegisteredEffects)
  {
    effect->setParameterSetNode(ParameterSetNode);
...
//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::showSourceVolumeInSliceViewers(vtkCollection* sliceLogics, bool forceShowInBackground /*=false*/, bool fitSlice /*=false*/)
{
...
//---------------------------------------------------------------------------
vtkSegmentEditorAbstractEffect* vtkSegmentEditorLogic::effectByName(std::string name)
{
  if (name.empty())
  {
    return nullptr;
  ...
  for (auto currentEffect : RegisteredEffects)
  {
    if (currentEffect->name() == name)
    {
...
//------------------------------------------------------------------------------
std::vector<std::string> vtkSegmentEditorLogic::effectNameOrder() const
{
  return EffectNameOrder;
}

//------------------------------------------------------------------------------
void vtkSegmentEditorLogic::setEffectNameOrder(const std::vector<std::string>& effectNames)
{
  if (effectNames == EffectNameOrder)
  {
    // no change
    return;
  }
  EffectNameOrder = effectNames;
  InvokeEvent(EffectListChangedEvent);
}

//------------------------------------------------------------------------------
int vtkSegmentEditorLogic::effectColumnCount() const
{
  return EffectColumnCount;
}

//------------------------------------------------------------------------------
void vtkSegmentEditorLogic::setEffectColumnCount(int columnCount)
{
  EffectColumnCount = columnCount;
  InvokeEvent(EffectListChangedEvent);
}

//------------------------------------------------------------------------------
bool vtkSegmentEditorLogic::unorderedEffectsVisible() const
{
  return UnorderedEffectsVisible;
}

//------------------------------------------------------------------------------
void vtkSegmentEditorLogic::setUnorderedEffectsVisible(bool visible)
{
  if (visible == UnorderedEffectsVisible)
  {
    // no change
    return;
  }
  UnorderedEffectsVisible = visible;
  InvokeEvent(EffectListChangedEvent);
}

//---------------------------------------------------------------------------
std::vector<std::string> vtkSegmentEditorLogic::availableEffectNames()
{
  std::vector<std::string> availableEffectNames;
  for (auto effect : RegisteredEffects)
  {
    availableEffectNames.emplace_back(effect->name());
  }
  return availableEffectNames;
}

//---------------------------------------------------------------------------
int vtkSegmentEditorLogic::effectCount()
{
  return RegisteredEffects.size();
}

//---------------------------------------------------------------------------
vtkSegmentEditorAbstractEffect* vtkSegmentEditorLogic::effectByIndex(int index)
{
  ...
  return RegisteredEffects[index];
}
...
SegmentEditorEventObservation vtkSegmentEditorLogic::createInteractorEventObservation(vtkSmartPointer<vtkSegmentEditorEventCallbackCommand> interactionCallbackCommand)
{
  // Connect interactor events
  vtkRenderWindowInteractor* interactor = interactionCallbackCommand->ViewRenderWindow->GetInteractor();
...
//---------------------------------------------------------------------------
SegmentEditorEventObservation vtkSegmentEditorLogic::createViewNodeModifiedEventObservation(vtkSmartPointer<vtkSegmentEditorEventCallbackCommand> interactionCallbackCommand)
{
...
//---------------------------------------------------------------------------
SegmentEditorEventObservation vtkSegmentEditorLogic::createSlicePoseModifiedEventObservation(vtkSmartPointer<vtkSegmentEditorEventCallbackCommand> interactionCallbackCommand)
{
...
//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::setupViewObservations(std::vector<vtkRenderWindow*> viewRenderWindows, std::vector<vtkMRMLAbstractViewNode*> viewNodes)
{
  if ((viewRenderWindows.size() != viewNodes.size()))
  {
    vtkErrorMacro("Inconsistent view render window to view nodes call.");
    return;
  }

  for (size_t i_view = 0; i_view < viewRenderWindows.size(); i_view++)
  {
    auto interactionCallback = createViewInteractionCallbackCommand(viewRenderWindows[i_view], viewNodes[i_view]);
...
//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::setupViewObservations(std::vector<vtkRenderWindow*> sliceRenderWindows,
                                                  ...
                                                  std::vector<vtkMRMLAbstractViewNode*> threeDViewNodes)
{
  ...
  setupViewObservations(sliceRenderWindows, sliceNodes);
  setupViewObservations(threeDViewRenderWindows, threeDViewNodes);
...
{
  for (SegmentEditorEventObservation eventObservation : EventObservations)
  {
...
//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::setActiveEffectByName(std::string effectName)
{
  vtkSegmentEditorAbstractEffect* effect = this->effectByName(effectName);
...
//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::hideLabelLayer(vtkCollection* sliceLogics)
{
  if (!sliceLogics)
  {
    return;
...
//---------------------------------------------------------------------------
bool vtkSegmentEditorLogic::turnOffLightboxes(vtkCollection* sliceLogics)
{
  if (!sliceLogics)
  {
    return false;
...
//---------------------------------------------------------------------------
bool vtkSegmentEditorLogic::autoShowSourceVolumeNode() const
{
  return AutoShowSourceVolumeNode;
}

//---------------------------------------------------------------------------
void vtkSegmentEditorLogic::setAutoShowSourceVolumeNode(bool autoShow)
{
  if (AutoShowSourceVolumeNode == autoShow)
  {
    return;
  }
  AutoShowSourceVolumeNode = autoShow;
}
