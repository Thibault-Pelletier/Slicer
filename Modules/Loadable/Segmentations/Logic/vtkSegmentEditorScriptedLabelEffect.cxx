...
#include "vtkSegmentEditorScriptedLabelEffect.h"
...
//-----------------------------------------------------------------------------
class vtkSegmentEditorScriptedLabelEffectPrivate
{
public:
  typedef vtkSegmentEditorScriptedLabelEffectPrivate Self;
  vtkSegmentEditorScriptedLabelEffectPrivate();
  virtual ~vtkSegmentEditorScriptedLabelEffectPrivate();
...
//-----------------------------------------------------------------------------
// vtkSegmentEditorScriptedLabelEffectPrivate methods

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedLabelEffectPrivate::vtkSegmentEditorScriptedLabelEffectPrivate()
{
...
//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedLabelEffectPrivate::~vtkSegmentEditorScriptedLabelEffectPrivate() = default;

//-----------------------------------------------------------------------------
// vtkSegmentEditorScriptedLabelEffect methods

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedLabelEffect::vtkSegmentEditorScriptedLabelEffect(QObject* parent)
  : Superclass(parent)
  , d_ptr(new vtkSegmentEditorScriptedLabelEffectPrivate)
{
  this->m_Name = QString("UnnamedScriptedLabelEffect");
}

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedLabelEffect::~vtkSegmentEditorScriptedLabelEffect() = default;

//-----------------------------------------------------------------------------
QString vtkSegmentEditorScriptedLabelEffect::pythonSource() const
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
  return d->PythonSourceFilePath;
}

//-----------------------------------------------------------------------------
bool vtkSegmentEditorScriptedLabelEffect::setPythonSource(const QString filePath)
{
  Q_D(vtkSegmentEditorScriptedLabelEffect);
  ...
  if (!classToInstantiate)
  {
    ...
    if (!vtkScriptedUtils::loadSourceAsModule(moduleName, filePath, global_dict, local_dict))
    {
      return false;
    }
  ...
  if (!classToInstantiate)
  {
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    QString("vtkSegmentEditorScriptedLabelEffect::setPythonSource - "
  ...
  if (!vtkScriptedUtils::setModuleAttribute("slicer", className, self))
  {
...
//-----------------------------------------------------------------------------
PyObject* vtkSegmentEditorScriptedLabelEffect::self() const
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::setName(QString name)
{
  this->m_Name = name;
}

//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::setPerSegment(bool perSegment)
{
  Q_UNUSED(perSegment);
...
//-----------------------------------------------------------------------------
QIcon vtkSegmentEditorScriptedLabelEffect::icon()
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
...
//-----------------------------------------------------------------------------
const QString vtkSegmentEditorScriptedLabelEffect::helpText() const
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
  ...
  {
    qWarning() << d->PythonSourceFilePath << ": vtkSegmentEditorScriptedLabelEffect: Function 'helpText' is expected to return a string!";
    return this->Superclass::helpText();
  }
...
//-----------------------------------------------------------------------------
vtkSegmentEditorAbstractEffect* vtkSegmentEditorScriptedLabelEffect::clone()
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::activate()
{
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::deactivate()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::setupOptionsFrame()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
QCursor vtkSegmentEditorScriptedLabelEffect::createCursor(qMRMLWidget* viewWidget)
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
...
//-----------------------------------------------------------------------------
bool vtkSegmentEditorScriptedLabelEffect::processInteractionEvents(vtkRenderWindowInteractor* callerInteractor, unsigned long eid, qMRMLWidget* viewWidget)
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::processViewNodeEvents(vtkMRMLAbstractViewNode* callerViewNode, unsigned long eid, qMRMLWidget* viewWidget)
{
  Q_D(const vtkSegmentEditorScriptedLabelEffect);
  PyObject* arguments = PyTuple_New(3);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::setMRMLDefaults()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::referenceGeometryChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::sourceVolumeNodeChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::masterVolumeNodeChanged()
{
  // Note: deprecated
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::layoutChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::interactionNodeModified(vtkMRMLInteractionNode* interactionNode)
{
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::updateGUIFromMRML()
{
  if (!this->active())
  {
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedLabelEffect::updateMRMLFromGUI()
{
  // Base class implementation needs to be called before the effect-specific one
