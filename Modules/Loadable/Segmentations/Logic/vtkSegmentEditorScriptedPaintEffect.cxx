...
#include "vtkSegmentEditorScriptedPaintEffect.h"
...
//-----------------------------------------------------------------------------
class vtkSegmentEditorScriptedPaintEffectPrivate
{
public:
  typedef vtkSegmentEditorScriptedPaintEffectPrivate Self;
  vtkSegmentEditorScriptedPaintEffectPrivate();
  virtual ~vtkSegmentEditorScriptedPaintEffectPrivate();
...
//-----------------------------------------------------------------------------
// vtkSegmentEditorScriptedPaintEffectPrivate methods

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedPaintEffectPrivate::vtkSegmentEditorScriptedPaintEffectPrivate()
{
...
//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedPaintEffectPrivate::~vtkSegmentEditorScriptedPaintEffectPrivate() = default;

//-----------------------------------------------------------------------------
// vtkSegmentEditorScriptedPaintEffect methods

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedPaintEffect::vtkSegmentEditorScriptedPaintEffect(QObject* parent)
  : Superclass(parent)
  , d_ptr(new vtkSegmentEditorScriptedPaintEffectPrivate)
{
  this->m_Name = QString("UnnamedScriptedPaintEffect");
}

//-----------------------------------------------------------------------------
vtkSegmentEditorScriptedPaintEffect::~vtkSegmentEditorScriptedPaintEffect() = default;

//-----------------------------------------------------------------------------
QString vtkSegmentEditorScriptedPaintEffect::pythonSource() const
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
  return d->PythonSourceFilePath;
}

//-----------------------------------------------------------------------------
bool vtkSegmentEditorScriptedPaintEffect::setPythonSource(const QString filePath)
{
  Q_D(vtkSegmentEditorScriptedPaintEffect);
  ...
  if (!classToInstantiate)
  {
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    QString("vtkSegmentEditorScriptedPaintEffect::setPythonSource - "
...
//-----------------------------------------------------------------------------
PyObject* vtkSegmentEditorScriptedPaintEffect::self() const
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::setName(QString name)
{
  this->m_Name = name;
}

//-----------------------------------------------------------------------------
QIcon vtkSegmentEditorScriptedPaintEffect::icon()
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
...
//-----------------------------------------------------------------------------
const QString vtkSegmentEditorScriptedPaintEffect::helpText() const
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
  ...
  {
    qWarning() << d->PythonSourceFilePath << ": vtkSegmentEditorScriptedPaintEffect: Function 'helpText' is expected to return a string!";
    return this->Superclass::helpText();
  }
...
//-----------------------------------------------------------------------------
vtkSegmentEditorAbstractEffect* vtkSegmentEditorScriptedPaintEffect::clone()
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::activate()
{
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::deactivate()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::setupOptionsFrame()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
QCursor vtkSegmentEditorScriptedPaintEffect::createCursor(qMRMLWidget* viewWidget)
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::setMRMLDefaults()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::referenceGeometryChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::sourceVolumeNodeChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::masterVolumeNodeChanged()
{
  // Note: deprecated
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::layoutChanged()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::updateGUIFromMRML()
{
  if (!this->active())
  {
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::updateMRMLFromGUI()
{
  // Base class implementation needs to be called before the effect-specific one
...
//-----------------------------------------------------------------------------
void vtkSegmentEditorScriptedPaintEffect::paintApply(qMRMLWidget* viewWidget)
{
  Q_D(const vtkSegmentEditorScriptedPaintEffect);
  PyObject* arguments = PyTuple_New(1);
  PyTuple_SET_ITEM(arguments, 0, PythonQtConv::QVariantToPyObject(QVariant::fromValue<QObject*>((QObject*)viewWidget)));
